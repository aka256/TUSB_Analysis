...
3. `WindWall`が0以上であるプレイヤーの座標で効果音を鳴らす
4. `NinjaTime`が0以上であるプレイヤーが存在したら、`NinjaTime`から1を引いて、以下の事柄を実行
   1. `NinjaTime`が-1以下であるプレイヤーの`Tsuremai`を0に設定
   2. `NinjaTime`が-1以下であるプレイヤーの`Choyaku`を0に設定
   3. `NinjaTime`が-1以下であるプレイヤーの`NinjaTime`を0に設定
5. `MagicShield`を持つエンティティに耐性Lv.4を15秒間付与
