...
<span class="label label-primary">位置 (-1920,14,-188)</span>

## About

`TypeCheck`を持ったエンティティに対して、特定のタグの付与や野良の村人の処理、TNT、クリーパーの即爆破処理、そして攻略率に対する難易度上昇処理などを行っている。

{% include important.html content="このコマンド群の中には[難易度調整](difficultyAdjustment.html)から**直接コマンドを書き換えられるコマンドがある**ため、ここをいじる場合には注意すべし" %}
