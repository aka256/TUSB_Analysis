...
- `RaiseEnd`が付与されているプレイヤーが存在するとき、[レイズリスポーン処理]({{site.baseurl}}/command/xCircuit10/xCircuit10_raiseWarp.html)を実行
